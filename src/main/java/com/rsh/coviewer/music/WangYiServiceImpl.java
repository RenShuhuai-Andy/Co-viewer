package com.rsh.coviewer.music;

import com.alibaba.fastjson.JSONObject;
import com.rsh.coviewer.music.bean.*;
import com.rsh.coviewer.music.entity.*;
import com.rsh.coviewer.music.service.WangYiService;
import com.rsh.coviewer.music.thread.MusicRunnable;
import com.rsh.coviewer.music.thread.MusicThread;
import com.rsh.coviewer.redis.IRedisUtils;
import com.rsh.coviewer.springdata.WangYiAlbumRepository;
import com.rsh.coviewer.springdata.WangYiLrcRepository;
import com.rsh.coviewer.springdata.WangYiMusicRepository;
import com.rsh.coviewer.springdata.WangYiSingerRepository;
import com.rsh.coviewer.springdata.entity.WangyialbumEntity;
import com.rsh.coviewer.springdata.entity.WangyilrcEntity;
import com.rsh.coviewer.springdata.entity.WangyimusicEntity;
import com.rsh.coviewer.springdata.entity.WangyisingerEntity;
...
import static com.rsh.coviewer.music.WangYiTypeEnum.*;
...
public class WangYiServiceImpl implements WangYiService {
    ...
    private void init(String name) {
        ...
        DATA.put("limit", "10");
        DATA.put("offset", "0");
        DATA.put("type", WangYiTypeEnum.SINGLE);
...
//        String list = redisUtils.get("wangyi_music_" + name);
        ...
        //暂时只开放根据歌名查找
        switch (type) {
            case WangYiTypeEnum.SINGLE:
                DATA.put("type", WangYiTypeEnum.SINGLE);
                break;
            case WangYiTypeEnum.ALBUM:
                DATA.put("type", WangYiTypeEnum.ALBUM);
                return null;
            case WangYiTypeEnum.SINGER:
                DATA.put("type", WangYiTypeEnum.SINGER);
                return null;
            default:
                return null;
        }
        WangYiBean bean;
        try {
            bean = HttpUnits.urlToBean(WangYiTypeEnum.GET_URL, WangYiBean.class, DATA, HEADERS, null, HttpMethodType.POST);
        } catch (Exception e) {
            e.printStackTrace();
            return responseEntity;
        }
    ...
    public BaseEntity getMusicComments(long song_id) {
        ...
        String result = redisUtils.hget(WangYiTypeEnum.COMMENT_CACHE , String.valueOf(song_id));
        ...
        String params;
        try {
            params = URLEncoder.encode(AES.getParams(String.valueOf(WangYiTypeEnum.FIRST_PARAM_START) + song_id + WangYiTypeEnum.FIRST_PARAM_END), "UTF-8");
        } catch (Exception e) {
            e.printStackTrace();
            return entity;
        }
        String encSecKey = AES.getEncSecKey();
        String url = WangYiTypeEnum.COMMENTS_URL_START + song_id + WangYiTypeEnum.COMMENTS_URL_END + "&params=" + params + "&encSecKey=" + encSecKey;
        try {
        ...
        entity.setCode(200);
        entity.setMsg("success");
        System.out.println(redisUtils.hset(WangYiTypeEnum.COMMENT_CACHE , String.valueOf(song_id), Tool.getInstance().toJson(commentBean), Time.LONG));
        return entity;
    }
    ...
    private String getMusicPicture(Long song_id) {
        WangYiResult result;
        try {
            result = HttpUnits.urlToBean(WangYiTypeEnum.MUSIC_PIC + "id" + song_id + "&ids=%5B" + song_id + "%5D", WangYiResult.class);
    ...
    public BaseEntity getHottingMusic() {
        ...
        return getHot(WangYiTypeEnum.HOTTING_MUSIC);
    }
    ...
    public BaseEntity getHotMusic() {
        ...
        return getHot(WangYiTypeEnum.HOT_MUSIC);
    }
    ...
    public BaseEntity getNewMusic() {
        ...
        return getHot(WangYiTypeEnum.NEW_MUSIC);
    }
    ...
    public BaseEntity getMusicUrlById(long song_id) {
        ...
        try {
            songDetail = HttpUnits.urlToBean(WangYiTypeEnum.GET_MUSIC + "&params=" + params + "&encSecKey=" + encSecKey, WangYiSongDetail.class, DATA, HEADERS, null, HttpMethodType.POST);
...
//                如果该歌曲数据库或者缓存已经存在了，则直接获取数据库的歌曲
                ...
                if (Tool.getInstance().isNullOrEmpty(song_url)) {
                    ...
                    if (Tool.getInstance().isNotNull(music)) {
                        ...
                        WangYiSongDetail songDetail;
                        //解密
                        String params = URLEncoder.encode(AES.getParams(String.valueOf(WangYiTypeEnum.FIRST_PARAM_START) + song_id + WangYiTypeEnum.FIRST_PARAM_END), "UTF-8");
                        String encSecKey = AES.getEncSecKey();
...
//            redisUtils.set("wangyi_music_" + name, Tool.getInstance().toJson(responseEntity));
    ...
    public String getMusicLyric(long song_id) {
        String lyric;
        //从缓存中取
        lyric = redisUtils.hget(WangYiTypeEnum.LYRIC_CACHE, String.valueOf(song_id));
...
//            System.out.println(result);
            ...
            if (object.containsKey("lrc")){
                ...
                if (lrc.containsKey("lyric")) {
                    ...
                    //保存缓存
                    redisUtils.hset(WangYiTypeEnum.LYRIC_CACHE, String.valueOf(song_id), lyric, Time.ONE_DAY);
                    return lyric;
                }
            }
            return null;
...
//            WangYiBean movie = HttpUnits.urlToBean(GET_URL, WangYiBean.class, DATA, HEADERS, null, HttpMethodType.POST);
//            long song_id = movie.getResult().getSongs()[0].getId();
